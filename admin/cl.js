document.addEventListener("DOMContentLoaded", function () {
    ...
    function calculateLoanEligibility(netIncome, otherEmi, loanTenure) {
        var adjustedNetIncome = netIncome * 0.4;
        var adjustedotherEmi = otherEmi * 0.4;

        return Math.round((adjustedNetIncome - adjustedotherEmi) * loanTenure);
    }

    function displayDetails() {
        let r = parseFloat(R) / 12 / 100;
        let n = parseFloat(N) * 12;
        let netIncomePerMonth = parseInt(document.getElementById("net-income").value);
        let otherEmi = parseInt(document.getElementById("other-emi").value);
        let netIncome = netIncomePerMonth;
        let totalEligibleFinalAmount = calculateLoanEligibility(netIncome, otherEmi, n);
        let num = parseFloat(totalEligibleFinalAmount) * parseFloat(r) * Math.pow(1 + parseFloat(r), n);
        let denom = Math.pow(1 + parseFloat(r), n) - 1;
    ...
    document.getElementById("loan-form-submit").addEventListener("click", function (e) {
        e.preventDefault();
        displayDetails();
    });
});